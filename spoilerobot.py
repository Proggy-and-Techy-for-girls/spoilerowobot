...
class TelegramBot:
    class attribute_dict:
        ...
        def __getattr__(self, index):
            if index == "__data__":
                return object.__getattr__(self, "__data__")
            try:
        ...
        def __setattr__(self, index, value):
            if index == "__data__":
                return object.__setattr__(self, "__data__", value)
        ...
        def __delattr__(self, index, value):
            if index == "__data__":
                return object.__delattr__(self, "__data__", value)
    ...
    def func_wrapper(self, fname):
        def func(self, unsafe, **kw):
            ...
            url = (
                ...
                + "/"
                + (
                    fname.replace("__UNSAFE", "")
                    if fname.endswith("__UNSAFE")
                    else fname
                )
                + "?"
...
def save(reason):
    ...
    try:
        (
            ...
            copy.deepcopy(SPOILERS),
            copy.deepcopy(SPOILERD),
            copy.deepcopy(SPOILERM),
            copy.deepcopy(TSPOILERS),
            copy.deepcopy(TSPOILERU),
            copy.deepcopy(PSPOILERU),
            copy.deepcopy(PPHASE),
            copy.deepcopy(PSPOILERS),
            copy.deepcopy(LSPOILERV),
            copy.deepcopy(XSPOILERS),
            copy.deepcopy(XSPOILERI),
        )
        (
            SPOILERS_,
            SPOILERD_,
            ...
            superdictify(SPOILERS_),
            superdictify(SPOILERD_),
            superdictify(SPOILERM_),
            superdictify(TSPOILERS_),
            superdictify(TSPOILERU_),
            superdictify(PSPOILERU_),
            superdictify(PPHASE_),
            superdictify(PSPOILERS_),
            superdictify(LSPOILERV_),
            superdictify(XSPOILERS_),
            superdictify(XSPOILERI_),
        )
            ...
            if update.has("inline_query"):
                                ...
                                if V in SPOILERS.keys():
                                    if True:
                                        if True or TSPOILERU[iqu] == V:
                                            if (
                                                type(SPOILERS[V]) == list
                                                or type(SPOILERS[V]) == dict
                                            ):
                                                appearance = None
                                                ...
                                                results = []
                                                single = V in SPOILERM.keys() and bool(
                                                    SPOILERM[V]
                                                )
                                                try:
                                                    if not SPOILERS[V][0].startswith(
                                                        "http"
                                                    ):
                                                        raise ValueError
                                                                    ...
                                                                    [
                                                                        {
                                                                            "text": "Show spoiler"
                                                                            if single
                                                                            else "Double tap to show spoiler",
                                                                            "callback_data": V,
                                                                        }
                                                                    ]
                                                                ]
                    ...
                    results.append(
                        {
                            ...
                            "thumb_height": 512,
                            "input_message_content": {
                                "message_text": "<i>Error: Too long for inline</i>",
                                "parse_mode": "HTML",
                            },
                        }
                    )
                                    ...
                                    results.append(
                                        {
                                            ...
                                            "input_message_content": {
                                                "message_text": "<b>Major Spoiler:</b> <pre>"
                                                + title
                                                + "</pre>",
                                                "parse_mode": "HTML",
                                            },
                                    ...
                                    results.append(
                                        {
                                            ...
                                            "description": "URL, custom title",
                                            "input_message_content": {
                                                "message_text": "<i>Minor Spoiler:</i> <pre>"
                                                + title
                                                + "</pre>",
                                                "parse_mode": "HTML",
                                            },
                            ...
                            results.append(
                                {
                                    ...
                                    "input_message_content": {
                                        "message_text": "<b>Major Spoiler!</b>",
                                        "parse_mode": "HTML",
                                    },
                            ...
                            results.append(
                                {
                                    ...
                                    "description": "URL, single-tap",
                                    "input_message_content": {
                                        "message_text": "<i>Minor Spoiler</i>",
                                        "parse_mode": "HTML",
                                    },
                    ...
                    results.append(
                        {
                            ...
                            "reply_markup": {
                                "inline_keyboard": [
                                    [
                                        {
                                            "text": "Double tap to show spoiler",
                                            "callback_data": key,
                                        }
                                    ]
                                ]
                    ...
                    results.append(
                        {
                            ...
                            "reply_markup": {
                                "inline_keyboard": [
                                    [{"text": "Show spoiler", "callback_data": key}]
                                ]
                            },
                        }
                    )
                ...
                if iqu in TSPOILERU.keys():
                    ...
                    if key in TSPOILERS.keys() and key not in SPOILERS.keys():
                        temp = TSPOILERS[key]
                        if (
                            key in TSPOILERC.keys()
                            and update.chosen_inline_result.result_id in TSPOILERC[key]
                        ):
                            temp = temp.split(":::", 1)[1]
                        SPOILERS[key] = temp
                        SPOILERD[key] = time.time()
                        if (
                            iqu in TSPOILERM.keys()
                            and TSPOILERM[iqu] == update.chosen_inline_result.result_id
                        ):
                            SPOILERM[key] = True
                        if (
                            iqu in TCSPOILERM.keys()
                            and TCSPOILERM[iqu] == update.chosen_inline_result.result_id
                        ):
                            SPOILERM[key] = True
                        del TSPOILERS[key]
                    del TSPOILERU[iqu]
                ...
                if type(SPOILERS[cqd]) == list and len(SPOILERS[cqd][0]) <= 200:
                    bot.answerCallbackQuery(
                        callback_query_id=cq.id, text=SPOILERS[cqd][0], show_alert=True
                    )
                elif type(SPOILERS[cqd]) == list:
                    try:
                        bot.sendMessage__UNSAFE(chat_id=uid, text=SPOILERS[cqd][0])
                        bot.answerCallbackQuery(
                            callback_query_id=cq.id,
                    ...
                    try:
                        if True:
                            obj = SPOILERS[cqd]
                            if obj["type"] == "Photo":
                                bot.sendPhoto__UNSAFE(
                                    chat_id=uid,
                                    photo=obj["file_id"],
                                    caption=None
                                    if "caption" not in obj.keys()
                                    else obj["caption"],
                                )
                            elif obj["type"] == "Audio":
                                bot.sendAudio__UNSAFE(
                                    chat_id=uid,
                                    audio=obj["file_id"],
                                    duration=None
                                    if "duration" not in obj.keys()
                                    else obj["duration"],
                                    performer=None
                                    if "performer" not in obj.keys()
                                    else obj["performer"],
                                    title=None
                                ...
                                bot.sendSticker__UNSAFE(
                                    chat_id=uid, sticker=obj["file_id"]
                                )
                            elif obj["type"] == "Video":
                                bot.sendVideo__UNSAFE(
                                    chat_id=uid,
                                    video=obj["file_id"],
                                    duration=None
                                    if "duration" not in obj.keys()
                    ...
                    try:
                        bot.sendMessage__UNSAFE(chat_id=uid, text=SPOILERS[cqd])
                        bot.answerCallbackQuery(
                            callback_query_id=cq.id,
                            text="The spoiler has been sent to you as a direct message.",
                        )
                    ...
                    bot.answerCallbackQuery(
                        callback_query_id=cq.id, text=SPOILERS[cqd], show_alert=True
                    )
                ...
                if update.message.has("text"):
                    ...
                    if (
                            ...
                            if type(SPOILERS[key]) == list:
                                ...
                                try:
                                    ...
                                    if obj["type"] == "Photo":
                                        ...
                                        bot.sendAudio(
                                            chat_id=chat_id, sticker=obj["file_id"]
                                        )
                                    elif obj["type"] == "Video":
                                        bot.sendVideo(
                                            chat_id=chat_id,
                                    ...
                                    bot.sendMessage(
                                        chat_id=chat_id,
                                        text="Sorry, an error occurred. Please report this to "
                                        + BOT_OWNER_TAG
                                        + " if the error persists.",
                                    )
                    elif umt.lower() == "/clear":
                        bot.sendMessage(chat_id=chat_id, text=(250 * ".\n"))
                    elif (
                        umt.lower() == "/cancel"
                        and update.message["from"].id in PPHASE.keys()
                    ):
                        ...
                        if phase == 0:
                            if len(umt) > 1024:
                                bot.sendMessage(
                                    chat_id=chat_id, text="This spoiler is too long."
                                )
                            else:
                                XSPOILERS[uid] = umt
                                bot.sendMessage(
                            ...
                            if umt in checklist:
                                ...
                                bot.sendMessage(
                                    chat_id=chat_id, text="Unknown spoiler type."
                                )
                        elif phase == 2:
                            if len(umt) > 100:
                                bot.sendMessage(
                                    chat_id=chat_id, text="The given title is too long."
                                )
                            else:
                                PSPOILERS[uid] = (
                    ...
                    if phase == 0:
                        ...
                        if obj != None:
                            ...
                            print(update)
                            bot.sendMessage(
                                chat_id=chat_id, text="Unrecognized media type."
                            )
        saferes = True
        time.sleep(0.1)
except KeyboardInterrupt as e:
